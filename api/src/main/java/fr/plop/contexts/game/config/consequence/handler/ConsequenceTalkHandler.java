...
public class ConsequenceTalkHandler implements ConsequenceHandler {
    ...
    public void message(GameInstanceContext context, I18n message) {
    ...
    public void talk(GameInstanceContext context, TalkItem.Id talkId) {
        PushEvent event = new PushEvent.Talk(context, talkId);
        pushPort.push(event);
    }

    public void confirm(GameInstanceContext context, Consequence.DisplayConfirm confirm) {
        GamePlayerEntity player = playerRepository.findByIdFetchUser(context.playerId().value()).orElseThrow();
        String message = confirm.message().value(player.getUser().getLanguage());
        PushEvent event = new PushEvent.Confirm(context, confirm.token(), message);
        pushPort.push(event);
    }
