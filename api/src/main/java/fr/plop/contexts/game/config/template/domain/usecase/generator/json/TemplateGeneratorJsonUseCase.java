...
public class TemplateGeneratorJsonUseCase {
    ...
    public GameProject.Code code(TemplateGeneratorRoot root) {
        return new GameProject.Code(root.code().toUpperCase());
    }

    public Game.Version version(TemplateGeneratorRoot root) {
        String version = root.version() == null ? "0.0.0" : root.version();
        return new Game.Version(version);
    }

    public Presentation presentation(TemplateGeneratorRoot root) {
        Presentation.Level level = root.level() == 0 ? Presentation.Level._default() : Presentation.Level.from(root.level());
        String label = root.label() == null ? "" : root.label();
        String description = root.description() == null ? "" : root.description();
        ...
        return new Presentation(new Presentation.Id(), new I18n(Map.of(Language.FR, label)), new I18n(Map.of(Language.FR, description)),
                level, Presentation.Visibility.SHOWN, departure,
                List.of(Presentation.GameType.TREASURE_HUNT), null, 5,
                List.of(Presentation.ParticipantType.SOLO, Presentation.ParticipantType.FRIENDS, Presentation.ParticipantType.FAMILY),
                List.of(), List.of());
    }

    public Template template(TemplateGeneratorRoot root) throws JsonProcessingException {

        //final Template.Atom templateAtom = new Template.Atom(, new Template.Code(root.code().toUpperCase()));
        ...
        ScenarioConfig scenario = mapScenario(root.scenario(), talk);
        ...
        return new Template(new Template.Id(), duration, scenario,
    ...
    private BoardSpace mapBoardSpace(TemplateGeneratorRoot.Board.Spaces spaceRoot) {
        BoardSpace.Id id = spaceRoot.ref() != null
                ? globalCache.reference(spaceRoot.ref(), BoardSpace.Id.class, new BoardSpace.Id(spaceRoot.ref()))
                : new BoardSpace.Id();
    ...
    private ScenarioConfig mapScenario(TemplateGeneratorRoot.Scenario scenarioRoot, TalkConfig talkConfig) {
        if (scenarioRoot == null || scenarioRoot.steps() == null) {
            return new ScenarioConfig();
        }
        List<ScenarioConfig.Step> steps = scenarioRoot.steps().stream()
                .map(step -> mapStep(step, talkConfig))
                .collect(Collectors.toList());
        return new ScenarioConfig(steps);
    }

    private ScenarioConfig.Step mapStep(TemplateGeneratorRoot.Scenario.Step stepRoot, TalkConfig talkConfig) {
        ScenarioConfig.Step.Id id = stepRoot.ref() != null
                ? globalCache.reference(stepRoot.ref(), ScenarioConfig.Step.Id.class, new ScenarioConfig.Step.Id(stepRoot.ref()))
        ...
        List<Possibility> possibilities = stepRoot.possibilities() == null ? List.of() : stepRoot.possibilities().stream()
                .map(p -> mapPossibility(p, id, talkConfig))
                .toList();
    ...
    private ScenarioConfig.Target mapTarget(TemplateGeneratorRoot.Scenario.Step.Target targetRoot) {
        ScenarioConfig.Target.Id id = targetRoot.ref() != null
                ? globalCache.reference(targetRoot.ref(), ScenarioConfig.Target.Id.class, new ScenarioConfig.Target.Id(targetRoot.ref()))
    ...
    private Possibility mapPossibility(TemplateGeneratorRoot.Scenario.Step.Possibility possibilityRoot, ScenarioConfig.Step.Id currentStepId, TalkConfig talkConfig) {
        PossibilityRecurrence recurrence = mapRecurrence(possibilityRoot.recurrence());
        PossibilityTrigger trigger = mapTrigger(possibilityRoot.trigger(), currentStepId, talkConfig);
    ...
    private PossibilityTrigger mapTrigger(TemplateGeneratorRoot.Scenario.Step.Possibility.Trigger triggerRoot, ScenarioConfig.Step.Id currentStepId, TalkConfig talkConfig) {
        if (triggerRoot == null) return null;
        return switch (triggerRoot.type().toUpperCase()) {
            case "GOINSPACE" -> {
                String spaceRef = triggerRoot.value() != null ? triggerRoot.value() : (String) triggerRoot.metadata().get("spaceId");
                BoardSpace.Id spaceId = globalCache.reference(spaceRef, BoardSpace.Id.class, new BoardSpace.Id(spaceRef));
            ...
            case "ABSOLUTETIME" ->
                    new PossibilityTrigger.AbsoluteTime(new PossibilityTrigger.Id(), GameInstanceTimeUnit.ofMinutes(Integer.parseInt(triggerRoot.value())));
            case "STEPACTIVE" -> new PossibilityTrigger.StepActive(new PossibilityTrigger.Id(), currentStepId);
            case "TALKINPUTTEXT" -> {
                TalkItem.Id talkId = globalCache.reference(triggerRoot.talkId(), TalkItem.Id.class, new TalkItem.Id(triggerRoot.talkId()));
            ...
            case "TALKOPTIONSELECT" -> {
                ...
                TalkItemNext.Options.Option.Id optionId = globalCache.reference(optionRef, TalkItemNext.Options.Option.Id.class, new TalkItemNext.Options.Option.Id(optionRef));
            ...
            case "TALKEND", "TALK" -> {
                ...
                TalkItem.Id talkId = globalCache.reference(talkRef, TalkItem.Id.class, new TalkItem.Id(talkRef));
                yield new PossibilityTrigger.Talk(new PossibilityTrigger.Id(), talkId);
            }
            case "CONFIRMANSWER", "MESSAGECONFIRM" -> {
                ...
                MessageToken token = globalCache.reference(confirmRef, MessageToken.class, new MessageToken(confirmRef));
                yield new PossibilityTrigger.MessageConfirmAnswer(new PossibilityTrigger.Id(), token, expectedAnswer);
            }
            case "AND" -> {
                List<PossibilityTrigger> childTriggers = triggerRoot.children().stream()
                        .map(child -> mapTrigger(child, currentStepId, talkConfig))
                        .toList();
            ...
            case "NOT" -> {
                PossibilityTrigger childTrigger = mapTrigger(triggerRoot.children().getFirst(), currentStepId, talkConfig);
                yield new PossibilityTrigger.Not(new PossibilityTrigger.Id(), childTrigger);
            }
            case "INVENTORYITEMACTION" -> {
                String itemRef = triggerRoot.value() != null ? triggerRoot.value() : (String) triggerRoot.metadata().get("itemId");
                ...
                yield new PossibilityTrigger.InventoryItemAction(new PossibilityTrigger.Id(), itemId);
            }
            case "IMAGEOBJECTCLICK" -> {
                String objectRef = triggerRoot.value() != null ? triggerRoot.value() : (String) triggerRoot.metadata().get("objectId");
                ImageObject.Id objectId = new ImageObject.Id(objectRef);
                yield new PossibilityTrigger.ImageObjectClick(new PossibilityTrigger.Id(), objectId);
            }
            default -> null;
        };
    }
    ...
    private Condition mapCondition(TemplateGeneratorRoot.Condition conditionRoot) {
        ...
        return switch (conditionRoot.type().toUpperCase()) {
            ...
            case "NOT" -> {
                Condition child = mapCondition(conditionRoot.children().getFirst());
                yield new Condition.Not(new Condition.Id(), child);
            }
            case "INSIDESPACE" -> {
            ...
            case "ABSOLUTETIME" -> {
                ...
                yield new Condition.AbsoluteTime(new Condition.Id(), GameInstanceTimeUnit.ofMinutes(duration), beforeOrAfter);
            }
            case "INSTEP" -> {
            ...
            case "TALKWITH" -> {
                String characterRef = (String) conditionRoot.metadata().get("characterId");
                TalkCharacter.Id characterId = globalCache.reference(characterRef, TalkCharacter.Id.class, new TalkCharacter.Id(characterRef));
                yield new Condition.TalkWithCharacter(new Condition.Id(), characterId);
            }
            case "INVENTORY", "INVENTORY_HAS" -> {
                String itemRef = (String) conditionRoot.metadata().get("itemId");
                GameConfigInventoryItem.Id itemId = globalCache.reference(itemRef, GameConfigInventoryItem.Id.class, new GameConfigInventoryItem.Id(itemRef));
                yield new Condition.InventoryHasItem(new Condition.Id(), itemId);
            }
            default -> null;
        };
    }
    ...
    private Consequence mapConsequence(TemplateGeneratorRoot.Consequence consequenceRoot) {
        ...
        return switch (consequenceRoot.type().toUpperCase()) {
            ...
            case "CONFIRM" -> {
                ...
                yield new Consequence.DisplayConfirm(new Consequence.Id(), message, new MessageToken(confirmRef));
            }
            case "INVENTORY_ADD", "INVENTORYADD" -> {
                String itemRef = (String) consequenceRoot.metadata().get("itemRef");
                GameConfigInventoryItem.Id itemId = globalCache.reference(itemRef, GameConfigInventoryItem.Id.class, new GameConfigInventoryItem.Id(itemRef));
                yield new Consequence.InventoryAddItem(new Consequence.Id(), itemId);
            }
            default -> null;
        };
    }
        ...
        if (talkRoot.characters() != null) {
            for (TemplateGeneratorRoot.Talk.Character charRoot : talkRoot.characters()) {
                if (charRoot.ref() != null) {
                    TalkCharacter character = new TalkCharacter(new TalkCharacter.Id(charRoot.ref()), charRoot.ref());
    ...
    private TalkItem mapTalkItem(TemplateGeneratorRoot.Talk.Item itemRoot, Map<String, TalkCharacter> characters, Map<String, Map<String, TemplateGeneratorRoot.Talk.Character.CharacterImage>> characterImages) {
        TalkItem.Id id = itemRoot.ref() != null
                ? globalCache.reference(itemRoot.ref(), TalkItem.Id.class, new TalkItem.Id(itemRoot.ref()))
                : new TalkItem.Id();
        ...
        TalkCharacter character = characters.getOrDefault(characterName, new TalkCharacter(new TalkCharacter.Id(characterName), characterName));
        ...
        if (itemRoot.inputText() != null) {
            ...
            for (int i = 0; i < itemRoot.options().size(); i++) {
                ...
                TalkItemNext.Options.Option.Id optionId = optionRoot.ref() != null
                        ? globalCache.reference(optionRoot.ref(), TalkItemNext.Options.Option.Id.class, new TalkItemNext.Options.Option.Id(optionRoot.ref()))
                ...
                if (optionRoot.next() != null) {
                    optNextId = Optional.of(globalCache.reference(optionRoot.next(), TalkItem.Id.class, new TalkItem.Id(optionRoot.next())));
                }
            ...
            if (itemRoot.next() != null) {
                optNextId = Optional.of(globalCache.reference(itemRoot.next(), TalkItem.Id.class, new TalkItem.Id(itemRoot.next())));
            }
    ...
    private TalkItemOut mapTalkOut(Object valueRoot) {
        System.out.println("DEBUG: mapTalkOut valueRoot status: " + (valueRoot == null ? "null" : valueRoot.getClass().getName()));
        System.out.println("DEBUG: mapTalkOut valueRoot: " + valueRoot);
        if (valueRoot instanceof Map<?, ?> map) {
            // Check if it's a conditional value (has "default" key)
            if (map.containsKey("default")) {
                System.out.println("DEBUG: map has default");
                Map<String, String> defaultMap = (Map<String, String>) map.get("default");
                I18n defaultText = toI18n(defaultMap);
                System.out.println("DEBUG: defaultText: " + defaultText);
                ...
                if (map.containsKey("branches")) {
                    ...
                    for (Map<String, Object> branchMap : branchList) {
                        ...
                        System.out.println("DEBUG: branch order: " + order + ", condition: " + condition + ", branchText: " + branchText);
    ...
    private GameConfigInventoryItem mapInventoryItem(TemplateGeneratorRoot.Inventory.InventoryItem itemRoot) {
        ...
        if (itemRoot.linkTargetRef() != null) {
            optLinkTargetId = Optional.of(globalCache.reference(itemRoot.linkTargetRef(), ScenarioConfig.Target.Id.class, new ScenarioConfig.Target.Id(itemRoot.linkTargetRef())));
        }

        int initValue = itemRoot.initValue() != null ? itemRoot.initValue() : 0;

        GameConfigInventoryItem.ActionType actionType = GameConfigInventoryItem.ActionType.NONE;
        if(!StringTools.isEmpty(itemRoot.actionType())) {
            actionType = GameConfigInventoryItem.ActionType.valueOf(itemRoot.actionType());
        }

        return new GameConfigInventoryItem(id, label, image, optDescription, type, initValue, optLinkTargetId, actionType);
    }

    /*private GameConfigInventoryItemActionRule mapActionRule(TemplateGeneratorRoot.Inventory.InventoryItem.ActionRule ruleRoot) {
        GameConfigInventoryItemActionRule.Type status = GameConfigInventoryItemActionRule.Type.valueOf(ruleRoot.status().toUpperCase());
        ...
        GameConfigInventoryItemActionRule.Consequence consequence = new GameConfigInventoryItemActionRule.Consequence.Direct(consequences);
        return new GameConfigInventoryItemActionRule(status, consequence);
    }*/
