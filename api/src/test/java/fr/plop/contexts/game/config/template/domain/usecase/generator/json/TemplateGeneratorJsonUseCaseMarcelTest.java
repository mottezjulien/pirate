...
public class TemplateGeneratorJsonUseCaseMarcelTest {
    ...
    public void marcel_with_moods() throws Exception {
        ...
        Template template = generator.template(TemplateGeneratorRootParser.apply(json));
        ...
        assertThat(firstMessage.out().resolve(new GameInstanceSituation()).value(Language.FR)).isEqualTo("Bonjour l'ami. Alors, c'est aujourd'hui le grand jour, cette fois, tu trouveras le trésor, tu réussiras là où j'ai échoué.");
        ...
        assertThat(secondMessage.out().resolve(new GameInstanceSituation()).value(Language.FR)).isEqualTo("Comme prévu, voici ma partie de la carte du trésor.");
        ...
        assertThat(thirdMessage.out().resolve(new GameInstanceSituation()).value(Language.FR)).isEqualTo("Sophie et Émilie possèdent les autres parties de la carte. Tu devrais leur parler afin de compléter la carte.");
        ...
        assertThat(fifthMessage.out().resolve(new GameInstanceSituation()).value(Language.FR)).isEqualTo("Émilie se trouve dans sa librairie.");
        ...
        assertThat(homeWithCard.out().resolve(new GameInstanceSituation()).value(Language.FR)).isEqualTo("Oui l'ami, que puis-je faire pour toi ?");
        ...
        assertThat(talkMarcel1.out().resolve(new GameInstanceSituation()).value(Language.FR)).contains("Tu dois parler avec Sophie et Émilie");
